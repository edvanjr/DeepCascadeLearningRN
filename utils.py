...
from keras_preprocessing.image import DirectoryIterator, Iterator, transform_matrix_offset_center, flip_axis
import numpy as np
import re
...
class NumpyArrayIterator(Iterator):
    ...
    def next(self):
        ...
        with self.lock:
            ...
            current_index = next(self.index_generator)
            current_batch_size = next(self.index_generator)
        
        # The transformation of images is not under thread lock so it can be done in parallel
        batch_x = np.zeros(tuple([current_batch_size[0]] + list(self.X.shape)[1:])).tolist()
...
class ImageDataGeneratorForCascading(object):
    ...
    def random_transform(self, x):
        ...
        x = self.apply_transform(x=x, transform_matrix=transform_matrix, channel_axis=img_channel_index,
